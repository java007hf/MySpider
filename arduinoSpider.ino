...
/* Size of the robot ---------------------------------------------------------*/
const float length_a = 75;
const float length_b = 65;
const float length_c = 32;
const float length_side = 71;
const float z_absolute = -28;
/* Constants for movement ----------------------------------------------------*/
const float z_default = -40, z_up = -30, z_boot = z_absolute;
const float x_default = 72, x_offset = 0;
const float y_start = 20, y_step = 40;
...
//functions' parameter
const float KEEP = 250;
//define PI for calculation
const float pi = 3.1415926;
/* Constants for turn --------------------------------------------------------*/

const float y_turn_step = 30;
//temp length
const float temp_a = sqrt(pow(2 * x_default + length_side, 2) + pow(y_turn_step, 2));
const float temp_b = 2 * (y_start + y_turn_step) + length_side;
const float temp_c = sqrt(pow(2 * x_default + length_side, 2) + pow(2 * y_start + y_turn_step + length_side, 2));
const float temp_alpha = acos((pow(temp_a, 2) + pow(temp_b, 2) - pow(temp_c, 2)) / 2 / temp_a / temp_b);
//site for turn
const float turn_x1 = (temp_a - length_side) / 2;
const float turn_y1 = y_start + y_turn_step / 2;
...
void turn_left(unsigned int step)
{
  ...
  while (step-- > 0)
  {
    if (site_now[3][1] == y_start)
    {
      ...
      set_site(2, x_default - x_offset, y_start + y_turn_step, z_default);
      set_site(3, x_default - x_offset, y_start + y_turn_step, z_default);
      wait_all_reach();
    ...
    else
    {
      ...
      set_site(0, x_default - x_offset, y_start + y_turn_step, z_default);
      set_site(1, x_default - x_offset, y_start + y_turn_step, z_default);
...
void polar_to_servo(int leg, float alpha, float beta, float gamma)
{
  ...
  setAngle(servo_pin[leg][0], beta);
  setAngle(servo_pin[leg][1], alpha);
  setAngle(servo_pin[leg][2], gamma);
}

void setAngle(uint8_t num,uint8_t angle)
{
  double pulse = angle; 
  pulse = pulse / 90 + 0.5;  //将角度映射为 0.5 到 2.5ms 的脉冲宽度 
  setServoPulse(num, pulse);  //0 到 180 度映射为 0.5 到 2.5ms 
}
